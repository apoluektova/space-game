...
function animate() {
                ...
                if (projectile.position.y - projectile.radius <= invader.position.y + invader.height
                    ...
                    setTimeout(() => {
                        ...
                        if (invaderFound && projectileFound) {
                            if (invader.position.y + invader.height >= canvas.height || invader.position.y + invader.height > player.position.y) {
                                setTimeout(() => {
                                    game.over = true
                                    game.active = false
                                }, 0)
    ...
    if (keys.leftArrow.pressed && player.position.x >= 0) {
        player.velocity.x = -7
        player.rotation = -0.15
    } else if (keys.rightArrow.pressed && player.position.x + player.width <= canvas.width) {
        player.velocity.x = 7
        player.rotation = 0.15
    }
    else {
